<?php
    ...
    class maaliyahControllerGenerate
    {
        protected $maaliyah;
        var $modulename = "maaliyah";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($maaliyah, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->maaliyah = $maaliyah;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`ip_address` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getTahun(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getBulan(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getNo_anggota(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getNama_anggota(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getInfaq(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getZakat(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getExternal(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getCreated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getUpdated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getUpdated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->maaliyah->getIp_address(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE maaliyah SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->maaliyah->getId(),$this->dbh)."',";
	    $sql .= "`tahun` = '".$this->toolsController->replacecharSave($this->maaliyah->getTahun(),$this->dbh)."',";
	    $sql .= "`bulan` = '".$this->toolsController->replacecharSave($this->maaliyah->getBulan(),$this->dbh)."',";
	    $sql .= "`no_anggota` = '".$this->toolsController->replacecharSave($this->maaliyah->getNo_anggota(),$this->dbh)."',";
	    $sql .= "`nama_anggota` = '".$this->toolsController->replacecharSave($this->maaliyah->getNama_anggota(),$this->dbh)."',";
	    $sql .= "`infaq` = '".$this->toolsController->replacecharSave($this->maaliyah->getInfaq(),$this->dbh)."',";
	    $sql .= "`zakat` = '".$this->toolsController->replacecharSave($this->maaliyah->getZakat(),$this->dbh)."',";
	    $sql .= "`external` = '".$this->toolsController->replacecharSave($this->maaliyah->getExternal(),$this->dbh)."',";
	    $sql .= "`created_date` = '".$this->toolsController->replacecharSave($this->maaliyah->getCreated_date(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->maaliyah->getCreated_by(),$this->dbh)."',";
	    $sql .= "`updated_date` = '".$this->toolsController->replacecharSave($this->maaliyah->getUpdated_date(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->maaliyah->getUpdated_by(),$this->dbh)."',";
	    $sql .= "`ip_address` = '".$this->toolsController->replacecharSave($this->maaliyah->getIp_address(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->maaliyah->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM maaliyah WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM maaliyah WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->maaliyah, $row);
            
            return $this->maaliyah;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM maaliyah where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  tahun like '%".$search."%'";
               $where .= "       or  bulan like '%".$search."%'";
               $where .= "       or  no_anggota like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM maaliyah  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM maaliyah  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            ...
            foreach ($this->dbh->query($sql) as $row) {
                    $maaliyah_ = new maaliyah();
                    $this->loadData($maaliyah_, $row);
                    $maaliyah_List[] = $maaliyah_;
            }
            return $maaliyah_List;            
        }

                
        function loadData($maaliyah,$row){
	    $maaliyah->setId($row['id']);
	    $maaliyah->setTahun($row['tahun']);
	    $maaliyah->setBulan($row['bulan']);
	    $maaliyah->setNo_anggota($row['no_anggota']);
	    $maaliyah->setNama_anggota($row['nama_anggota']);
	    $maaliyah->setInfaq($row['infaq']);
	    $maaliyah->setZakat($row['zakat']);
	    $maaliyah->setExternal($row['external']);
	    $maaliyah->setCreated_date($row['created_date']);
	    $maaliyah->setCreated_by($row['created_by']);
	    $maaliyah->setUpdated_date($row['updated_date']);
	    $maaliyah->setUpdated_by($row['updated_by']);
	    $maaliyah->setIp_address($row['ip_address']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                require_once './views/maaliyah/maaliyah_list.php';
            }else{
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $maaliyah_ = $this->showData($id);
                require_once './views/maaliyah/maaliyah_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $maaliyah_ = $this->showData($id);
                require_once './views/maaliyah/maaliyah_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $maaliyah_ = $this->showData($id);
                require_once './views/maaliyah/maaliyah_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $maaliyah_ = $this->showData($id);
                require_once './views/maaliyah/maaliyah_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $no_anggota = isset($_POST['no_anggota'])?$_POST['no_anggota'] : "";
	    $nama_anggota = isset($_POST['nama_anggota'])?$_POST['nama_anggota'] : "";
	    $infaq = isset($_POST['infaq'])?$_POST['infaq'] : "";
	    $zakat = isset($_POST['zakat'])?$_POST['zakat'] : "";
	    $external = isset($_POST['external'])?$_POST['external'] : "";
	    ...
	    $this->maaliyah->setId($id);
	    $this->maaliyah->setTahun($tahun);
	    $this->maaliyah->setBulan($bulan);
	    $this->maaliyah->setNo_anggota($no_anggota);
	    $this->maaliyah->setNama_anggota($nama_anggota);
	    $this->maaliyah->setInfaq($infaq);
	    $this->maaliyah->setZakat($zakat);
	    $this->maaliyah->setExternal($external);
	    $this->maaliyah->setCreated_date($created_date);
	    $this->maaliyah->setCreated_by($created_by);
	    $this->maaliyah->setUpdated_date($updated_date);
	    $this->maaliyah->setUpdated_by($updated_by);
	    $this->maaliyah->setIp_address($ip_address);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->maaliyah->getId());
            if($check == 0){
        ...
        public function getMaaliyah() {
            return $this->maaliyah;
        }
        public function setMaaliyah($maaliyah) {
            $this->maaliyah = $maaliyah;
        }
