<?php
    ...
    class master_materiControllerGenerate
    {
        protected $master_materi;
        var $modulename = "master_materi";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($master_materi, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_materi = $master_materi;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`updated_at` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getKode_materi(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getNama_materi(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getAlias_materi(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getLevel(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getCreated_at(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getUpdated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_materi->getUpdated_at(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_materi SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_materi->getId(),$this->dbh)."',";
	    $sql .= "`kode_materi` = '".$this->toolsController->replacecharSave($this->master_materi->getKode_materi(),$this->dbh)."',";
	    $sql .= "`nama_materi` = '".$this->toolsController->replacecharSave($this->master_materi->getNama_materi(),$this->dbh)."',";
	    $sql .= "`alias_materi` = '".$this->toolsController->replacecharSave($this->master_materi->getAlias_materi(),$this->dbh)."',";
	    $sql .= "`level` = '".$this->toolsController->replacecharSave($this->master_materi->getLevel(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->master_materi->getCreated_by(),$this->dbh)."',";
	    $sql .= "`created_at` = '".$this->toolsController->replacecharSave($this->master_materi->getCreated_at(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->master_materi->getUpdated_by(),$this->dbh)."',";
	    $sql .= "`updated_at` = '".$this->toolsController->replacecharSave($this->master_materi->getUpdated_at(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_materi->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_materi WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_materi WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_materi, $row);
            
            return $this->master_materi;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_materi where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  kode_materi like '%".$search."%'";
               $where .= "       or  nama_materi like '%".$search."%'";
               $where .= "       or  alias_materi like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_materi  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_materi  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_materi_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_materi_ = new master_materi();
                    $this->loadData($master_materi_, $row);
                    $master_materi_List[] = $master_materi_;
            }
            return $master_materi_List;            
        }

                
        function loadData($master_materi,$row){
	    $master_materi->setId($row['id']);
	    $master_materi->setKode_materi($row['kode_materi']);
	    $master_materi->setNama_materi($row['nama_materi']);
	    $master_materi->setAlias_materi($row['alias_materi']);
	    $master_materi->setLevel($row['level']);
	    $master_materi->setCreated_by($row['created_by']);
	    $master_materi->setCreated_at($row['created_at']);
	    $master_materi->setUpdated_by($row['updated_by']);
	    $master_materi->setUpdated_at($row['updated_at']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_materi_list = $this->showDataAllLimit();
                ...
                require_once './views/master_materi/master_materi_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_materi/master_materi_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_materi_ = $this->showData($id);
                require_once './views/master_materi/master_materi_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_materi_ = $this->showData($id);
                require_once './views/master_materi/master_materi_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_materi_ = $this->showData($id);
                require_once './views/master_materi/master_materi_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_materi_ = $this->showData($id);
                require_once './views/master_materi/master_materi_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $alias_materi = isset($_POST['alias_materi'])?$_POST['alias_materi'] : "";
	    ...
	    $this->master_materi->setId($id);
	    $this->master_materi->setKode_materi($kode_materi);
	    $this->master_materi->setNama_materi($nama_materi);
	    $this->master_materi->setAlias_materi($alias_materi);
	    $this->master_materi->setLevel($level);
	    $this->master_materi->setCreated_by($created_by);
	    $this->master_materi->setCreated_at($created_at);
	    $this->master_materi->setUpdated_by($updated_by);
	    $this->master_materi->setUpdated_at($updated_at);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_materi->getId());
            if($check == 0){
        ...
        public function getMaster_materi() {
            return $this->master_materi;
        }
        public function setMaster_materi($master_materi) {
            $this->master_materi = $master_materi;
        }
