<?php
    ...
    class financeControllerGenerate
    {
        protected $finance;
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($finance, $dbh) {
	    ...
	    $sql .= "`ip_address` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getTahun(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getBulan(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getNo_anggota(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getNama_anggota(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getInfaq(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getZakat(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getExternal(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getCreated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getUpdated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getUpdated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->finance->getIp_address(), $this->dbh)."' ";
	    ...
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->finance->getId(),$this->dbh)."',";
	    $sql .= "`tahun` = '".$this->toolsController->replacecharSave($this->finance->getTahun(),$this->dbh)."',";
	    $sql .= "`bulan` = '".$this->toolsController->replacecharSave($this->finance->getBulan(),$this->dbh)."',";
	    $sql .= "`no_anggota` = '".$this->toolsController->replacecharSave($this->finance->getNo_anggota(),$this->dbh)."',";
	    $sql .= "`nama_anggota` = '".$this->toolsController->replacecharSave($this->finance->getNama_anggota(),$this->dbh)."',";
	    $sql .= "`infaq` = '".$this->toolsController->replacecharSave($this->finance->getInfaq(),$this->dbh)."',";
	    $sql .= "`zakat` = '".$this->toolsController->replacecharSave($this->finance->getZakat(),$this->dbh)."',";
	    $sql .= "`external` = '".$this->toolsController->replacecharSave($this->finance->getExternal(),$this->dbh)."',";
	    $sql .= "`created_date` = '".$this->toolsController->replacecharSave($this->finance->getCreated_date(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->finance->getCreated_by(),$this->dbh)."',";
	    $sql .= "`updated_date` = '".$this->toolsController->replacecharSave($this->finance->getUpdated_date(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->finance->getUpdated_by(),$this->dbh)."',";
	    $sql .= "`ip_address` = '".$this->toolsController->replacecharSave($this->finance->getIp_address(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->finance->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        ...
        function showData($id){
            $sql = "SELECT * FROM finance WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";
        ...
        function checkData($id){
            $sql = "SELECT count(*) FROM finance where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM finance  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM finance  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            ...
            foreach ($this->dbh->query($sql) as $row) {
                    $finance_ = new finance();
                    $this->loadData($finance_, $row);
                    $finance_List[] = $finance_;
            }
            return $finance_List;            
        }

                
        function loadData($finance,$row){
	    $finance->setId($row['id']);
	    $finance->setTahun($row['tahun']);
	    $finance->setBulan($row['bulan']);
	    $finance->setNo_anggota($row['no_anggota']);
	    $finance->setNama_anggota($row['nama_anggota']);
	    $finance->setInfaq($row['infaq']);
	    $finance->setZakat($row['zakat']);
	    $finance->setExternal($row['external']);
	    $finance->setCreated_date($row['created_date']);
	    $finance->setCreated_by($row['created_by']);
	    $finance->setUpdated_date($row['updated_date']);
	    $finance->setUpdated_by($row['updated_by']);
	    $finance->setIp_address($row['ip_address']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $finance_list = $this->showDataAllLimit();
                ...
                require_once './views/finance/finance_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/finance/finance_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $finance_ = $this->showData($id);
                require_once './views/finance/finance_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $finance_ = $this->showData($id);
                require_once './views/finance/finance_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $finance_ = $this->showData($id);
                require_once './views/finance/finance_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        public function saveData(){
            $check = $this->checkData($this->finance->getId());
            if($check == 0){
