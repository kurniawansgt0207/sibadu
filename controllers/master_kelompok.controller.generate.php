<?php
    ...
    class master_kelompokControllerGenerate
    {
        protected $master_kelompok;
        var $modulename = "master_kelompok";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($master_kelompok, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_kelompok = $master_kelompok;
            $this->dbh = $dbh;            
        ...
        function insertData(){
            ...
            $sql = "INSERT INTO master_kelompok ";
            $sql .= " ( ";
	    $sql .= "`id`,";
	    $sql .= "`unitid`,";
	    $sql .= "`deptid`,";
	    ...
	    $sql .= "`ip_address` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getUnitid(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getDeptid(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getKelompok(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getCreated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getUpdated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getUpdated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_kelompok->getIp_address(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_kelompok SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_kelompok->getId(),$this->dbh)."',";
	    $sql .= "`unitid` = '".$this->toolsController->replacecharSave($this->master_kelompok->getUnitid(),$this->dbh)."',";
	    $sql .= "`deptid` = '".$this->toolsController->replacecharSave($this->master_kelompok->getDeptid(),$this->dbh)."',";
	    $sql .= "`kelompok` = '".$this->toolsController->replacecharSave($this->master_kelompok->getKelompok(),$this->dbh)."',";
	    $sql .= "`created_date` = '".$this->toolsController->replacecharSave($this->master_kelompok->getCreated_date(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->master_kelompok->getCreated_by(),$this->dbh)."',";
	    $sql .= "`updated_date` = '".$this->toolsController->replacecharSave($this->master_kelompok->getUpdated_date(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->master_kelompok->getUpdated_by(),$this->dbh)."',";
	    $sql .= "`ip_address` = '".$this->toolsController->replacecharSave($this->master_kelompok->getIp_address(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_kelompok->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_kelompok WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_kelompok WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_kelompok, $row);
            
            return $this->master_kelompok;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_kelompok where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_kelompok  ".$where;
            //$sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_kelompok  ".$where;
            //$sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_kelompok_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_kelompok_ = new master_kelompok();
                    $this->loadData($master_kelompok_, $row);
                    $master_kelompok_List[] = $master_kelompok_;
            }
            return $master_kelompok_List;            
        }

                
        function loadData($master_kelompok,$row){
	    $master_kelompok->setId($row['id']);
	    $master_kelompok->setUnitid($row['unitid']);
	    $master_kelompok->setDeptid($row['deptid']);
	    $master_kelompok->setKelompok($row['kelompok']);
	    $master_kelompok->setCreated_date($row['created_date']);
	    $master_kelompok->setCreated_by($row['created_by']);
	    $master_kelompok->setUpdated_date($row['updated_date']);
	    $master_kelompok->setUpdated_by($row['updated_by']);
	    $master_kelompok->setIp_address($row['ip_address']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_kelompok_list = $this->showDataAllLimit();
                ...
                require_once './views/master_kelompok/master_kelompok_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_kelompok/master_kelompok_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_kelompok_ = $this->showData($id);
                require_once './views/master_kelompok/master_kelompok_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_kelompok_ = $this->showData($id);
                require_once './views/master_kelompok/master_kelompok_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_kelompok_ = $this->showData($id);
                require_once './views/master_kelompok/master_kelompok_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_kelompok_ = $this->showData($id);
                require_once './views/master_kelompok/master_kelompok_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $this->master_kelompok->setId($id);
	    $this->master_kelompok->setUnitid($unitid);
	    $this->master_kelompok->setDeptid($deptid);
	    $this->master_kelompok->setKelompok($kelompok);
	    $this->master_kelompok->setCreated_date($created_date);
	    $this->master_kelompok->setCreated_by($created_by);
	    $this->master_kelompok->setUpdated_date($updated_date);
	    $this->master_kelompok->setUpdated_by($updated_by);
	    $this->master_kelompok->setIp_address($ip_address);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_kelompok->getId());
            if($check == 0){
        ...
        public function getMaster_kelompok() {
            return $this->master_kelompok;
        }
        public function setMaster_kelompok($master_kelompok) {
            $this->master_kelompok = $master_kelompok;
        }
