<?php
    ...
    class master_profilController extends master_profilControllerGenerate
    {
        function showAvatar(){
            ...
            $check_profil = $this->checkData($id);
            $master_profil = $this->showData($id);
            if($check_profil > 0){                                   
                $avatar_picture = $master_profil->getAvatar();            
            } else {
                $avatar_picture = "profile-icon.png";
            }

            echo "<img src=\"images_small/".$avatar_picture. "\" onClick=\"openForm('index.php?model=master_profil&action=showImageMedium&id=".$master_profil->getId()."')\" class=\"img-circle\" width=\"75\" height=\"75\" class=\"tooltip\" alt=\"User Image\"> ";
        }
            ...
            if(isset($_POST["submit"])) {
                ...
                if($check != false) {
                    ...
                    if ($uploadok) {
                        ...
                        $master_profil->setNik($master_user_session->getNik());
                        $master_profil->setUser($master_user_session->getUser());
                        $master_profil->setDepartmentid($master_user_session->getDepartmentid());
                        $master_profil->setUnitid($master_user_session->getUnitid());
                        $this->saveData();                                                
