<?php
    ...
    class master_status_keluargaControllerGenerate
    {
        protected $master_status_keluarga;
        var $modulename = "master_status_keluarga";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($master_status_keluarga, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_status_keluarga = $master_status_keluarga;
            $this->dbh = $dbh;            
        ...
        function insertData(){
            ...
            $sql = "INSERT INTO master_status_keluarga ";
            $sql .= " ( ";
	    $sql .= "`id`,";
	    $sql .= "`status_keluarga`,";
	    $sql .= "`created_date`,";
	    ...
	    $sql .= "`updated_by` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_status_keluarga->getStatus_keluarga(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_status_keluarga->getCreated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_status_keluarga->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_status_keluarga->getUpdated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_status_keluarga->getUpdated_by(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_status_keluarga SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_status_keluarga->getId(),$this->dbh)."',";
	    $sql .= "`status_keluarga` = '".$this->toolsController->replacecharSave($this->master_status_keluarga->getStatus_keluarga(),$this->dbh)."',";
	    $sql .= "`created_date` = '".$this->toolsController->replacecharSave($this->master_status_keluarga->getCreated_date(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->master_status_keluarga->getCreated_by(),$this->dbh)."',";
	    $sql .= "`updated_date` = '".$this->toolsController->replacecharSave($this->master_status_keluarga->getUpdated_date(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->master_status_keluarga->getUpdated_by(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_status_keluarga->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_status_keluarga WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_status_keluarga WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_status_keluarga, $row);
            
            return $this->master_status_keluarga;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_status_keluarga where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  status_keluarga like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_status_keluarga  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_status_keluarga  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_status_keluarga_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_status_keluarga_ = new master_status_keluarga();
                    $this->loadData($master_status_keluarga_, $row);
                    $master_status_keluarga_List[] = $master_status_keluarga_;
            }
            return $master_status_keluarga_List;            
        }

                
        function loadData($master_status_keluarga,$row){
	    $master_status_keluarga->setId($row['id']);
	    $master_status_keluarga->setStatus_keluarga($row['status_keluarga']);
	    $master_status_keluarga->setCreated_date($row['created_date']);
	    $master_status_keluarga->setCreated_by($row['created_by']);
	    $master_status_keluarga->setUpdated_date($row['updated_date']);
	    $master_status_keluarga->setUpdated_by($row['updated_by']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_status_keluarga_list = $this->showDataAllLimit();
                ...
                require_once './views/master_status_keluarga/master_status_keluarga_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_status_keluarga/master_status_keluarga_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_status_keluarga_ = $this->showData($id);
                require_once './views/master_status_keluarga/master_status_keluarga_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_status_keluarga_ = $this->showData($id);
                require_once './views/master_status_keluarga/master_status_keluarga_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_status_keluarga_ = $this->showData($id);
                require_once './views/master_status_keluarga/master_status_keluarga_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_status_keluarga_ = $this->showData($id);
                require_once './views/master_status_keluarga/master_status_keluarga_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $this->master_status_keluarga->setId($id);
	    $this->master_status_keluarga->setStatus_keluarga($status_keluarga);
	    $this->master_status_keluarga->setCreated_date($created_date);
	    $this->master_status_keluarga->setCreated_by($created_by);
	    $this->master_status_keluarga->setUpdated_date($updated_date);
	    $this->master_status_keluarga->setUpdated_by($updated_by);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_status_keluarga->getId());
            if($check == 0){
        ...
        public function getMaster_status_keluarga() {
            return $this->master_status_keluarga;
        }
        public function setMaster_status_keluarga($master_status_keluarga) {
            $this->master_status_keluarga = $master_status_keluarga;
        }
