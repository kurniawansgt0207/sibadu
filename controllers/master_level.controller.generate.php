<?php
    ...
    class master_levelControllerGenerate
    {
        protected $master_level;
        var $modulename = "master_level";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($master_level, $dbh) {
	    ...
	    $sql .= "`updated_by` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_level->getLevel(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_level->getCreated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_level->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_level->getUpdated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_level->getUpdated_by(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_level SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_level->getId(),$this->dbh)."',";
	    $sql .= "`level` = '".$this->toolsController->replacecharSave($this->master_level->getLevel(),$this->dbh)."',";
	    $sql .= "`created_date` = '".$this->toolsController->replacecharSave($this->master_level->getCreated_date(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->master_level->getCreated_by(),$this->dbh)."',";
	    $sql .= "`updated_date` = '".$this->toolsController->replacecharSave($this->master_level->getUpdated_date(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->master_level->getUpdated_by(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_level->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        ...
        function showData($id){
            $sql = "SELECT * FROM master_level WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_level, $row);
            
            return $this->master_level;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_level where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_level  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_level  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_level_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_level_ = new master_level();
                    $this->loadData($master_level_, $row);
                    $master_level_List[] = $master_level_;
            }
            return $master_level_List;            
        }

                
        function loadData($master_level,$row){
	    $master_level->setId($row['id']);
	    $master_level->setLevel($row['level']);
	    $master_level->setCreated_date($row['created_date']);
	    $master_level->setCreated_by($row['created_by']);
	    $master_level->setUpdated_date($row['updated_date']);
	    $master_level->setUpdated_by($row['updated_by']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_level_list = $this->showDataAllLimit();
                ...
                require_once './views/master_level/master_level_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_level/master_level_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_level_ = $this->showData($id);
                require_once './views/master_level/master_level_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_level_ = $this->showData($id);
                require_once './views/master_level/master_level_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_level_ = $this->showData($id);
                require_once './views/master_level/master_level_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_level_ = $this->showData($id);
                require_once './views/master_level/master_level_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $this->master_level->setId($id);
	    $this->master_level->setLevel($level);
	    $this->master_level->setCreated_date($created_date);
	    $this->master_level->setCreated_by($created_by);
	    $this->master_level->setUpdated_date($updated_date);
	    $this->master_level->setUpdated_by($updated_by);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_level->getId());
            if($check == 0){
        ...
        public function getMaster_level() {
            return $this->master_level;
        }
        public function setMaster_level($master_level) {
            $this->master_level = $master_level;
        }
